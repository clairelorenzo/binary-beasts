...
const Data = {
  ...
  data() {
    return {
      chartData: {
        labels: xVals.value,
        datasets: [{ data: yVals.value, backgroundColor: "rgba(78, 112, 163, 0.6)", borderColor: "rgba(203, 220, 245, 1)", borderWidth: 2, label: "Num Tasks Completed" }],
      },
      chartOptions: {
        responsive: true,
        plugins: {
...
<template>
  <Bar id="tracking-chart" :options="Data.data().chartOptions" :data="Data.data().chartData" />
</template>
